  ...
  const handleTopicSubmit = async (selectedTopic?: string) => {
    ...
    try {
      const { materials: generatedMaterials, visualContext: context, locationName: generatedLoc, searchSources: sources } = 
        await generateMaterialList(query, userLocation || undefined, uploadedImage || undefined, isThinkingMode);
    ...
    const sourcesHtml = searchSources.length > 0 
      ? `<div style="margin-top:24px; padding-top:16px; border-top:1px solid #e4e4e7;">
           ...
           <ul style="margin:0; padding-left:16px; font-size:10px; color:#a1a1aa;">
             ${searchSources.map(s => `<li><a href="${s.web.uri}" style="color:#71717a; text-decoration:none;">${s.web.title}</a> (from ${s.web.sourceType || 'Web'})</li>`).join('')}
             ${searchSources.some(s => s.maps) ? '<li>Contains Google Maps Data</li>' : ''}
           </ul>
         </div>`
      : '';
                ...
                <div className={`flex items-center justify-between p-2 px-3 border-t ${inputBar}`}>
                    <div className="flex items-center gap-3">
                        ...
                        <button
                           ...
                           className={`flex items-center gap-1.5 px-2 py-1 rounded text-xs font-medium transition-colors ${isThinkingMode ? 'text-indigo-500 bg-indigo-500/10' : uploadBtn}`}
                           title="Enable gemini-3-pro-preview for complex reasoning"
                        >
                           <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                           <span>Thinking: {isThinkingMode ? 'On' : 'Off'}</span>
                        </button>
                    </div>
                ...
                <div className={`mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[11px] ${textSubtitle}`}>
                   ...
                   {searchSources.map((source, idx) => (
                      <React.Fragment key={idx}>
                         ...
                         {source.maps && (
                             <span className="text-indigo-400">Maps: {source.maps.title}</span>
                         )}
                      </React.Fragment>
                   ))}
                </div>
